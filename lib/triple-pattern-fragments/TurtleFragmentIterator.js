...
function TurtleFragmentIterator(source, fragmentUrl) {
  if (!(this instanceof TurtleFragmentIterator)) return new TurtleFragmentIterator(source, fragmentUrl);
...
const fs = require("fs");
const ws = fs.createWriteStream("ldfResponse", { flags: "a" });
...
TurtleFragmentIterator.prototype._processTriple = function(triple) {
  ...
  if (triple.subject !== this._fragmentUrl && triple.predicate.indexOf(rdf.HYDRA) !== 0) this._push(triple);
  else this.metadataStream._push(triple);
};
