...
FragmentsClient.prototype.getFragmentByPattern = function(pattern) {
  ...
  if (startFragment.error !== null) {
    // null means definitely correctly loaded
    if (
      startFragment.error // an error means definitely incorrectly loaded
    )
  ...
  startFragment.getProperty("controls", function(controls) {
    ...
    var subject = rdf.isVariableOrBlank(pattern.subject)
      ? null
      : pattern.subject;
    var predicate = rdf.isVariableOrBlank(pattern.predicate)
      ? null
      : pattern.predicate;
...
function Fragment(fragmentsClient, pattern) {
  if (!(this instanceof Fragment)) return new Fragment(fragmentsClient);
...
Fragment.prototype.loadFromUrl = function(pageUrl) {
  ...
  var fragment = this,
    fragmentsClient = this._fragmentsClient,
    rawPage,
    headers = { "user-agent": "Triple Pattern Fragments Client" };
  if (fragmentsClient._startFragmentUrl)
    headers.referer = fragmentsClient._startFragmentUrl;
  ...
  rawPage.getProperty("statusCode", function(statusCode) {
    if (statusCode !== 200) {
      rawPage.emit(
        "error",
        new Error("Could not retrieve " + pageUrl + " (" + statusCode + ")")
      );
    ...
    rawPage.getProperty("contentType", function(contentType) {
      ...
      if (!Parser)
        return fragment.emit(
          "error",
          new Error("No parser for " + contentType + " at " + pageUrl)
        );
      var parsedPage = (fragment._page = new Parser(rawPage, pageUrl));
      parsedPage.on("readable", function() {
        fragment.readable = true;
      });

      // Extract the page's metadata and controls
      var controls = {};
      fragmentsClient._metadataExtractor.extract(
        { fragmentUrl: pageUrl },
        parsedPage.metadataStream,
...
Fragment.prototype.toString = function() {
  return (
    "[" + this.constructor.name + " {" + rdf.toQuickString(this._pattern) + ")}"
  );
};
